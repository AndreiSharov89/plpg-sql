...
--smotrim dannie tablic ishodnih dannih
select * from istochnik.tablesource;
select * from nashabaza.ourtable;
--gotovim vivod iz istochnika v vid tablici nashabaza
select 
...
--proveriaem max dati d tablicah
select max(dtenddate) from istochnik.tablesource;
select max(dtreportdate) from nashabaza.ourtable;
--vivodim stroki s max datoi iz istochnika
select 
	npokazatelid::int as ncanonid,
	dtenddate as dtreportdate,
	SPLIT_PART(vterritoryid, '_', 1)::int as nterotdelenie,
	...
	nvalue::decimal(6,4) as vprocent
from istochnik.tablesource
where dtenddate = (select max(dtenddate) from istochnik.tablesource);
--vivodim stroki iz istochnika s datoi > max dati nashabaza
select 
	npokazatelid::int as ncanonid,
	dtenddate as dtreportdate,
	SPLIT_PART(vterritoryid, '_', 1)::int as nterotdelenie,
	...
	nvalue::decimal(6,4) as vprocent
from istochnik.tablesource
where dtenddate > (select max(dtreportdate) from nashabaza.ourtable);
--create test table
CREATE TABLE nashabaza.ourtable_add (
propid bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL,
ncanonid int NOT NULL, -- номер показателя
dtreportdate date NOT NULL, -- дата конца отчетного периода
nterotdelenie int NOT NULL, -- ID региона
...
--add max-date-row from nasha table to test table
insert into 
		nashabaza.ourtable_add
			(ncanonid,dtreportdate,nterotdelenie,nterpodrazdel,vprocent)
		select 
			ncanonid,
			dtreportdate,
		...
		where dtreportdate = (select max(dtreportdate) from nashabaza.ourtable);
select * from nashabaza.ourtable_add;
truncate table nashabaza.ourtable_add restart identity;

--test on test-table
do
$$
declare
	ist date := (select max(dtenddate) from istochnik.tablesource);
	nasha date := (select max(dtreportdate) from nashabaza.ourtable_add);
begin
if ist > nasha then
	insert into 
		nashabaza.ourtable_add
	...
	raise notice 'Data changed';
else 
	raise notice 'Data not changed';
end if;
end;
$$
language 'plpgsql';
...
insert into 
		...
		where dtreportdate = (select max(dtreportdate) from nashabaza.ourtable);
select * from nashabaza.ourtable_add;
--moving to plpgsql function for task1 but on test table
--DROP FUNCTION task1(date,date);
create function task1(ist date, nasha date)
returns text
as
$$
...
if ist > nasha then
	insert into 
		...
		from istochnik.tablesource
		where dtenddate > nasha;
	f_output := 'Data changed';
	return f_output;
else 
	f_output := 'Data not changed';
	return f_output;
end if;
end;
$$ language plpgsql;
...
--call
select task1((select max(dtenddate) from istochnik.tablesource)::date,
	(select max(dtreportdate) from nashabaza.ourtable_add)::date);
select * from nashabaza.ourtable_add;
...
--create source and result test tables with data on equal dates
CREATE TABLE istochnik.tablesource_test2 (
...
INSERT INTO istochnik.tablesource_test2
(lineid, npokazatelid,vperiodtype,dtstartdate,dtenddate,vterritoryid,nvalue)
select *
from istochnik.tablesource
where dtenddate = '2023-06-30';
select * from istochnik.tablesource_test2;


CREATE TABLE nashabaza.ourtable_test2 (
...
INSERT INTO nashabaza.ourtable_test2
(propid,ncanonid,dtreportdate,nterotdelenie,nterpodrazdel,vprocent)
select *
from nashabaza.ourtable
where dtreportdate = '2023-06-30';
select * from nashabaza.ourtable_test2;

select count(*) from istochnik.tablesource_test2;
select count(*) from nashabaza.ourtable_test2;
...
--testing checksums
select md5(array_agg(f.*)::text)
		   from nashabaza.ourtable_test2 f;
select md5(array_agg(f.*)::text)
						from(select
							ncanonid,
							dtreportdate,
							...
							vprocent
							from nashabaza.ourtable_test2) f;
select md5(array_agg(f.*)::text)from(select
		npokazatelid::int as ncanonid,
		dtenddate as dtreportdate,
		SPLIT_PART(vterritoryid, '_', 1)::int as nterotdelenie,
		SPLIT_PART(vterritoryid, '_', 2)::int as nterpodrazdel,
		nvalue::decimal(6,4) as vprocent
		from istochnik.tablesource_test2) f;
...
begin
	if target = src then
		raise notice 'Данные на последнюю дату отчета синхронизированы';
	else 
		delete from nashabaza.ourtable_test2 where dtreportdate = '2023-06-30';
		INSERT INTO nashabaza.ourtable_test2
		(propid,ncanonid,dtreportdate,nterotdelenie,nterpodrazdel,vprocent)
		select 
		lineid::bigint as propid,
		npokazatelid::int as ncanonid,
		dtenddate as dtreportdate,
		SPLIT_PART(vterritoryid, '_', 1)::int as nterotdelenie,
		SPLIT_PART(vterritoryid, '_', 2)::int as nterpodrazdel,
		nvalue::decimal(6,4) as vprocent
		from istochnik.tablesource_test2;
	end if;
end;
$$
...
$$
declare 
	ist date := (select max(dtenddate) from istochnik.tablesource_test2)::date;
	nasha date := (select max(dtreportdate) from nashabaza.ourtable_test2)::date;
	target text := (select md5(array_agg(f.*)::text)
						from(select
							npokazatelid::int as ncanonid,
							dtenddate as dtreportdate,
...
select * from nashabaza.ourtable_test2;
--reset
truncate table nashabaza.ourtable_test2;
INSERT INTO nashabaza.ourtable_test2
(propid,ncanonid,dtreportdate,nterotdelenie,nterpodrazdel,vprocent)
select *
from nashabaza.ourtable
